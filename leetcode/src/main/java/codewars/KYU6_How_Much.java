 ...
 */
public class KYU6_How_Much {
	/**
	 * @author: ZhouJie
	 * @date: 2020年3月3日 上午1:42:35 
	 * @param: @param m
	 * @param: @param n
	 * @param: @return
	 * @return: String
	 * @Description: 1-找到x，x在[m,n]内，且 x%7=2 && x%9=1
	 * 
	 * 				可以加速，满足的x最小值是37，且下一个满足值总是在37基础上+63，如此即可在[m,n]内找到所有x
	 *
	 */
	public static String howmuch(int m, int n) {
		...
		int start = 37, step = 63;
		while (start < m) {
			start += step;
		}
		sb.append("[");
		while (start <= n) {
			sb.append("[M: " + start + " B: " + start / 7 + " C: " + start / 9 + "]");
			start += step;
		}
		return sb.length() < 10 ? "[]" : sb.append("]").toString();
	}
}
